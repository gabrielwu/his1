        ...
        <table class="tb1 tb2" id="tb_r" >
            ...
            <tbody>
				<tr>
					...
					<td align="center">
						<select name="status" id="status" class="" title='<?php echo ($status); ?>'>
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
						...
						<td align="center" class="operate">
							<a href="<?php echo ($root); ?>/operationTypeField/index/<?php echo ($data["id"]); ?>" class="" >手术字段</a>
...
function add(url) {
	...
	data.name = $("#name").val();
	var canSubmit = true;
	canSubmit = isNonEmpty(data.name, "手术名称", $("#name"));
	if (canSubmit) {
		operate(data, url);
	} 
}
function edit(id, url) {
	...
	var canSubmit = true;
	canSubmit = isNonEmpty(data.name, "手术名称", $("#name_" + id));
	if (canSubmit) {
		operate(data, url);
	} 
}
